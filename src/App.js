...
function useGrowthBook() {
  ...
  React.useEffect(() => {
    ...
    const uniqueUserId = setUniqueUserIdCookie(); // Generate import React from 'react';
import { GrowthBook, GrowthBookProvider, useFeatureIsOn } from '@growthbook/growthbook-react';
import logo from './logo.svg';
import './App.css';
...
function useGrowthBook() {
  ...
  React.useEffect(() => {
    ...
    const initGrowthBook = () => {
      ...
      // Set attributes with anonymous_id instead of UU
...
function CTAButton() {
  ...
  const handleClick = () => {
    ...
    window.dataLayer.push({
      ...
      anonymous_id: getUniqueUserId(), // Include unique_user_id as anonymous_id
    });
  };
...
// Utility function to retrieve unique_user_id from cookies
